...
# On affiche l'échantillon dédiés à l'apprentissage en orange et l'échantillon dédié au test en bleu
...
# attention : il faut tenir compte de la classe des variables testées
  log_loss <- - mean(log(prob_class[(class_pred == x$class_test)])) - mean(log(1 - prob_class[(class_pred == x$class_test)]))
  log_loss
```
...
D'après moi, pour $k = 10$, on trouve qu'il y a une meilleure précision sur la prédiction dans l'ensemble test. Mais pour $k = 13$, la perte est minimale dans $k \in ]1, 30]$.
...
```{r}
# 
  accuracy <- mean(is_it_orange == (prob_class > 0.5))
  accuracy
