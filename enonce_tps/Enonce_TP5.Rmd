...
```{r}
    ...
    # précision entre les 2 ensemble (apprentissage et test)
      accuracy_lda <- mean(pred_lda == y_test)
      
    # probabilité de l'ensemble testé
      prob_lda <- prediction$posterior
      
       barplot(t(prob_lda), col = c("lightblue", "orange"),
            border = NA,
            xlab = "Test set")
      which(log(prob_lda) < 0)
    # logloss 
      log_loss_lda <- -mean((y_test == 1)*log(1 - prob_lda) + (y_test == 0)*log(prob_lda))
...
model_keras %>% 
    layer_dense(units = 300, activation = 'softmax', input_shape = 2000) %>% 
    layer_dropout(rate = 0.1) %>% 
...
## On applique l'entrainement sur 20 générations
history_optimized <- model_keras %>% fit(
                    x = x_imbd, 
                    y = y_imbd, 
                    epochs = 10,
                    batch_size = 2,
...
```
```{r}
eval_keras_optimized <- model_keras %>% evaluate(x_test, y_test)
loss_keras_optimized <- eval_keras_optimized$loss
acc_keras_optimized <- eval_keras$acc
acc_keras_optimized
loss_keras_optimized
```
...
```{r}
  ...
  accuracy_nnet <- mean((prob_nnet > 0.5) == (y_test == 1))

  log_loss_nnet <- -mean((y_test == 0)*log(1 - prob_nnet) + (y_test == 1)*log(prob_nnet))
...
```{r}
    # knn
  mod_knn <- class::knn(x_train, 
                          x_train, 
                          y_train, 
                          k = 50, 
                          prob = TRUE)
    
  accuracy_knn <- mean(mod_knn == y_test)
  accuracy_knn
  
  prob_knn <- attr(mod_knn, "prob")
  prob_knn[prob_knn == 1] <-  1 - 1e-08
    
  log_loss_knn <- - mean( (mod_knn == y_train)*log(prob_knn) + (mod_knn != y_train)*log(1 - prob_knn) )
  log_loss_knn
``` 

```{r}
  log_loss <-  c(loss_keras, log_loss_lda, log_loss_nnet, log_loss_knn)

  names(log_loss) <-  c("keras", "lda", "nnet", 'knn')
  
  barplot(log_loss, col = 2:4, ylab = "log loss")
  
  accuracy <- c(acc_keras, accuracy_lda, accuracy_nnet, accuracy_knn)
  
  names(accuracy) <-  c("keras", "lda", "nnet", 'knn')
...
```{r}
# standard r packages: 
  # saveRDS(model, file = "nguyene_model.RDS")
