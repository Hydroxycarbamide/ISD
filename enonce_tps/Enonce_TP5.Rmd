...
# On retranche les données en ensembles de tests et d'apprentissage
  x_train <- x_imbd[(0:200),]
  x_test <- x_imbd[-(0:200),]
  y_train <- y_imbd[(0:200)]
  y_test <- y_imbd[-(0:200)]
```
...
```{r}
    ...
    # lda de l'ensemble d'apprentissage
      # mod_lda <- MASS::lda(x_train, data = x_train[,1])
    mod_lda <- MASS::lda(x = x_train, grouping = x_train[,1])

    # probabilité à priori de l'ensemble d'apprentissage
      prediction <- predict(mod_lda, newdata = x_train)
      pred_lda <- prediction$class
    # précision entre les 2 ensemble (apprentissage et test)
      accuracy_lda <- mean(pred_lda == x_train[,1])
